...
This allows methods like [hotspot=findByLengthGreaterThan file=0]`findByLengthGreaterThan(float length)` or [hotspot=findByHeightBetween file=0]`findByHeightBetween(float minHeight, float maxHeight)` to automatically be turned into queries. For more information see the JavaDoc section on https://jakarta.ee/specifications/data/1.0/apidocs/jakarta.data/module-summary#:~:text=Refer%20to%20the%20API%20documentation%20for%20%40Query%20for%20further%20information.-,Query%20by%20Method%20Name,-The%20Query%20by%20Method%20Name%20pattern%20translates[Query by Method Name]
...
The [hotspot=Find file=0]`@Find` annotation indicates that a method is a query which may return entities. Used in isolation it
returns all entities, but by using the [hotspot=By file=0]`@By` annotation, it can be limited to entities which match the value provided at runtime. The [hotspot=getPackagesArrivingIn file=0]`getPackagesArrivingIn` method demonstrates using the [hotspot=Find file=0]`@Find` and [hotspot=By file=0]`@By`
...
This method returns the shortest packages in order from shortest to longest, but limited to only the specified number of results.
