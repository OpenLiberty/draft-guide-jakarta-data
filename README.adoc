...
:page-description: Data Persistence with Jakarta Data
...
:page-seo-description: Data Persistence with Jakarta Data
...
This application allows you to run queries against different shipping package dimensions and destinations.  Go to the http://localhost:9080 URL to see all the application data. You will see a column of queries, some of which will accept input. To start find the `findAll` query with no inputs. Selecting this will return all of the query data, called packages, similar to the following:
...
----
#Update the `Package` class in the start directory:#
...
----
#Update the `Packages.java` class in the start directory:#
...
```java
// Example of using insert, save and delete on a package
...
For more information on all of the options available with Query by Method name, see the https://jakarta.ee/specifications/data/1.0/jakarta-data-addendum-1.0.pdf[Query by Method Name Extension] of the https://jakarta.ee/specifications/data/1.0/[Jakarta Data specification].
...
=== Sort
In addition to the `@OrderBy` annotation it is possible to provide sorting at runtime. This is accomplished by adding a `Sort` object as a parameter to your query method:
...
See the https://jakarta.ee/specifications/data/1.0/[Jakarta Data specification] for a more comprehensive overview of the Jakarta Data Query Language.
