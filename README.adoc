...
You will learn how to use Jakarta Data to store data as well as simple, yet powerful options it provides for querying data. 
TODO...

=== What is Jakarta Data?
Jakarta Data is a new data access specification being released with Jakarta EE 11.
TODO.,.
...
Jakarta Data provides the `Sort` and `Limit` to create queries which can be modified at runtime. It also includes a mechanism for paging results to better handle large sets of results.
...
//TODO the spec? Liberty doc?

== Where to next?
Try out the sample application by creating your own queries.
